                ...
                if[row, col] in self.boat_coordinates:
                    ...
                    if type(actions) is list:
                        for action in actions:
                            ...
                            if self.can_place_water_around(row, col, value, size):
...
class Bimaru(Problem):
    ...
    def result(self, state: BimaruState, action):
        ...
        for part in action["Parts"]:
            (row, col, va# bimaru.py: Template para implementação do projeto de Inteligência Artificial 2022/2023.
# Devem alterar as classes e funções neste ficheiro de acordo com as instruções do enunciado.
# Além das funções e classes já definidas, podem acrescentar outras que considerem pertinentes.
...
class Board:
    ...
    def calculate_state(self):
        ...
        ship_name = ship_mapping.get(size)
        if ship_name:
            total = self.ships[ship_name]
            if (5 - size) == total + len(self.possible_values):
                self.possible_values.append("Certo")
        return self
...
class Bimaru(Problem):
    ...
    def actions(self, state: BimaruState):
        ...
        try:
            if state.board.possible_values[-1] == "Certo":
                return [state.board.possible_values[:-1]]
            else:
                return state.board.possible_values
        except:
            return state.board.possible_values
    ...
    def goal_test(self, state: BimaruState):
        ...
        if len(state.board.complete_rows) != 10 or state.board.complete_rows != 10:
            return False
        elif state.board.ships['Couraçado'] != 1:
            return False
